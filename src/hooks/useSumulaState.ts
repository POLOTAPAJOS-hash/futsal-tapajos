...
export function useSumulaState() {
  ...
  const updatePlayer = (team: "a" | "b", id: number, updates: Partial<Player>) => {
    if (team === "a") {
      setPlayersA(prev => prev.map(p => p.id === id ? { ...p, ...updates } : p));
    } else {
      setPlayersB(prev => prev.map(p => p.id === id ? { ...p, ...updates } : p));
    }
  };
